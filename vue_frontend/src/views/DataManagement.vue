<template>
  <div class="data-management-container">
          ...
          <div class="collection-section">
            ...
            <div class="sources-toolbar">
              <el-button type="primary" @click="showAddSourceDialog"
                ><el-icon><Plus /></el-icon>&nbsp;添加数据源</el-button
              >
            ...
            >
              ...
              >
                <template #default="scope">{{
                  formatDate(scope.row.lastCollection)
                }}</template>
              </el-table-column>
              <el-table-column label="配置" min-width="220">
                <template #default="scope">
                  <el-tooltip
                    placement="top"
                    effect="dark"
                    :content="String(scope.row.config || '')"
                  >
                    <span class="config-ellipsis"
                      >{{ String(scope.row.config || "").slice(0, 50)
                      }}<span v-if="String(scope.row.config || '').length > 50"
                        >...</span
                      ></span
                    >
                  </el-tooltip>
              ...
              <el-table-column label="操作" width="300" fixed="right">
                <template #default="scope">
                  ...
                  <el-button
                    size="small"
                    type="primary"
                    :disabled="!scope.row.active || !(scope.row.id > 0)"
                    @click="collectData(scope.row.id)"
                    >立即采集</el-button
                  >
                  <el-button
                    type="text"
                    size="small"
                    @click="viewRuns(scope.row)"
                    >采集记录</el-button
                  >
                  <template v-if="scope.row.id > 0">
                    <el-popconfirm
                      ...
                      @confirm="deleteDataSource(scope.row)"
                    >
                      <template #reference>
            ...
            <div class="collection-tasks">
              <div class="tasks-header">
                ...
                <el-button
                  ...
                  type="danger"
                  plain
                  :disabled="(collectionTasks || []).length === 0"
                  @click="clearTasks"
                  >清空</el-button
                >
              </div>
                ...
                <el-table-column prop="progress" label="进度">
                  <template #default="scope">
                    <el-progress
                      ...
                      :status="getStatusProgress(scope.row.status)"
                    ></el-progress>
                  </template>
                </el-table-column>
                <el-table-column
      ...
      <div v-if="detailData">
        ...
        >
          ...
          <el-descriptions-item label="错误信息">
            <div v-if="detailData.message" class="msg-pre">
              {{ detailData.message }}
            </div>
            <span v-else>-</span>
          </el-descriptions-item>
    ...
    <!-- 添加数据源对话框 -->
    <el-dialog v-model="addSourceVisible" title="添加数据源" width="500px">
      <el-form ref="sourceForm" :model="newSource" :rules="sourceRules">
        ...
        <el-form-item label="配置信息" prop="config">
          <el-input
            v-model="newSource.config"
            type="textarea"
            placeholder='请输入JSON，如 {"table":"exam_scores","key_column":"id","interval_seconds":30}'
            rows="5"
          ></el-input>
          <div class="json-tools">
            <el-button type="text" @click="formatJson('new')"
              >格式化JSON</el-button
            >
            <el-button type="text" @click="applyTemplate('new', 'id')"
              >模板：主键增量</el-button
            >
            <el-button type="text" @click="applyTemplate('new', 'updated')"
              >模板：时间戳增量</el-button
            >
          </div>
        </el-form-item>
    ...
    <!-- 编辑数据源对话框 -->
    <el-dialog v-model="editSourceVisible" title="编辑数据源" width="500px">
      <el-form ref="editSourceForm" :model="editSource" :rules="sourceRules">
        <el-form-item label="数据源名称" prop="name">
          <el-input
        ...
        <el-form-item label="配置信息" prop="config">
          <el-input
            v-model="editSource.config"
            type="textarea"
            placeholder='请输入JSON，如 {"table":"exam_scores","key_column":"id","interval_seconds":30}'
            rows="5"
          ></el-input>
          <div class="json-tools">
            <el-button type="text" @click="formatJson('edit')"
              >格式化JSON</el-button
            >
            <el-button type="text" @click="applyTemplate('edit', 'id')"
              >模板：主键增量</el-button
            >
            <el-button type="text" @click="applyTemplate('edit', 'updated')"
              >模板：时间戳增量</el-button
            >
          </div>
        </el-form-item>
...
export default {
  ...
  setup() {
    ...
    const loadData = async () => {
      try {
        ...
        if (Array.isArray(dataSources.value) && dataSources.value.length > 4) {
          const allTemp = dataSources.value.every(
            (s) => !s || !s.id || s.id <= 0,
          );
    ...
    const submitNewSource = async () => {
      try {
        if (!validateJson(newSource.config)) {
          alert("配置信息必须是有效的JSON");
          return;
        }
    ...
    const submitEditSource = async () => {
      ...
      try {
        if (!validateJson(editSource.config)) {
          alert("配置信息必须是有效的JSON");
          return;
        }
    ...
    const applyTemplate = (mode, type) => {
      ...
      if (mode === "new") newSource.config = tpl;
      else editSource.config = tpl;
    };
    ...
    const viewRuns = async (source) => {
      try {
        ...
        const res = await axios.get("/api/analysis/collection-runs", {
          params: { source_id: source?.id, limit: 20 },
        });
    ...
    const simulateCollectionProgress = (taskId, hasServer = false) => {
      ...
      const interval = setInterval(() => {
        ...
        if (progress >= 100) {
          ...
          if (hasServer) {
            axios
              .put(`/api/analysis/collection-tasks/${taskId}`, {
                status: "completed",
                progress: 100,
              })
              .catch(() => {});
...
:deep(.upload-detail-desc .el-descriptions__body .el-descriptions__table) {
  table-layout: fixed;
  width: 100%;
}
