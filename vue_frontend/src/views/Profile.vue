<template>
  <div class="profile-container">
            ...
            <div class="security-section">
              <el-form label-width="120px" class="security-form">
                ...
                <el-form-item label="登录记录" class="security-item">
                  <el-button type="primary" @click="viewLoginHistory"
                    >查看登录记录</el-button
                  >
                </el-form-item>
              </el-form>
    ...
    <!-- 更换头像对话框 -->
    <el-dialog v-model="changeAvatarVisible" title="更换头像" width="400px">
    ...
    <!-- 登录记录对话框 -->
    <el-dialog v-model="loginHistoryVisible" title="登录记录" width="800px">
      ...
      try {
        ...
        if (response.data && response.data.status === "success") {
          ...
          if (
            ...
            this.$message.error(
              response.data?.message || "保存失败，请稍后再试",
            );
          }
        }
      } catch (error) {
        ...
        if (error.response) {
          ...
          if (error.response.status === 401) {
            ...
            this.$message.error(
              `请求失败: ${error.response.statusText || "未知错误"}`,
            );
          }
    ...
    async confirmAvatarChange() {
      try {
        ...
        if (uploadResponse.data.status === "success") {
          this.avatarUrl =
            uploadResponse.data.data.avatarUrl || this.previewAvatar;
          ...
          window.dispatchEvent(
            new CustomEvent("avatarChanged", { detail: this.avatarUrl }),
          );
