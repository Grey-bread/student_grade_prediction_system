    ...
    <el-card class="main-card">
      ...
      <el-form :inline="true" :model="query" class="query-form">
        <el-form-item label="成绩表">
          <el-select
            v-model="query.selectedGradeTable"
            placeholder="选择成绩表"
            style="width: 180px"
            @focus="fetchGradeTables"
          >
            <el-option
      ...
      <div v-else>
        ...
        <el-card class="section-card">
          ...
          <div class="info">
            <el-tag type="info"
              >ID:
              {{
                safeGet(detail, "profile.student_id", query.studentId)
              }}</el-tag
            >
            <el-tag>{{ safeGet(detail, "profile.name") }}</el-tag>
            <el-tag type="success">{{
              safeGet(detail, "profile.gender")
            }}</el-tag>
            <el-tag type="warning">{{
              safeGet(detail, "profile.grade")
            }}</el-tag>
            <el-tag type="primary">{{
              safeGet(detail, "profile.class")
            }}</el-tag>
          </div>
        </el-card>
        ...
        <div class="kpis">
          ...
          >
            ...
            <div v-if="item.sub" class="kpi-sub">
              分位：{{ formatScore(item.sub) }}%
            </div>
          </el-card>
        </div>

        <!-- 学习投入 -->
        <el-card v-if="detail.factors?.length" class="section-card">
          <template #header
            ><div class="section-header">学习投入</div></template
          >
          <div class="info">
            <el-tag
              v-for="f in detail.factors"
              :key="f.name"
              type="info"
              class="pill"
              >{{ f.name }}：{{ f.value ?? "—" }}</el-tag
            >
          </div>
        </el-card>
        ...
        <el-card class="section-card">
          <template #header
            ><div class="section-header">成绩趋势反馈</div></template
          >
          <div v-if="trendFeedback.length">
            <p v-for="(t, i) in trendFeedback" :key="i" style="margin: 6px 0">
              {{ t }}
            </p>
          </div>
        ...
        <el-card class="section-card">
          <template #header
            ><div class="section-header">薄弱环节定位</div></template
          >
          <div v-if="weaknessFeedback.length">
            <p
              v-for="(t, i) in weaknessFeedback"
              :key="i"
              style="margin: 6px 0"
            >
              {{ t }}
        ...
        <el-card class="section-card">
          <template #header
            ><div class="section-header">个性化提升建议</div></template
          >
          <div v-if="suggestionFeedback.length">
            <el-timeline>
              <el-timeline-item
                v-for="(s, idx) in suggestionFeedback"
                :key="idx"
                :timestamp="`建议 ${idx + 1}`"
                type="primary"
                >{{ s }}</el-timeline-item
        ...
        <el-card class="section-card">
          <template #header
            ><div class="section-header">历史记录</div></template
          >
...
export default {
  ...
  data() {
    return {
      ...
      classStats: {
        avgScore: null,
        avgHours: null,
        attemptAvg: { first: null, second: null, third: null },
      },
    };
  },
  computed: {
    ...
    kpiList() {
      ...
      return [
        {
          title: "高数平均分",
          value: g.calculus_avg_score,
          sub: this.detail.percentiles?.calculus_avg_score,
        },
  ...
  methods: {
    ...
    async onGradeTableChange(table) {
      ...
      // 预取新表数据（容错，不抛出）
      try {
        await this.ensureUGLoadedPreferBackend(table);
      } catch (e) {
        this.csvUG = [];
      }
    },
    formatScore(val) {
      return this.toNum(val) === null ? "—" : this.toNum(val).toFixed(1);
    },
    ...
    async ensureStudentsLoadedPreferBackend() {
      ...
      try {
        const res = await axios.get("/api/analysis/table-data", {
    ...
    async loadFeedback() {
      ...
      try {
        ...
        const gradesTable =
          this.query.selectedGradeTable || "university_grades";
        try {
          const res = await axios.get("/api/analysis/student-detail", {
            params: { student_id: this.query.studentId, table: gradesTable },
          });
          if (res.data?.status === "success") {
            usedBackend = true;
            this.detail = {
        ...
        if (!usedBackend)
          this.detail = this.buildDetailFromCsv(
            String(this.query.studentId).trim(),
          );
    ...
    async saveFeedback() {
      ...
      try {
        ...
        const res = await axios.post(
          "/api/analysis/student-feedback/save",
          payload,
        );
        if (res.data?.status === "success")
          this.$message?.success?.("已保存到系统");
    ...
    async loadHistory() {
      try {
        ...
        const r = await axios.get("/api/analysis/student-feedback/history", {
          params: { student_id: this.query.studentId, limit: 100 },
        });
        if (r.data?.status === "success")
          this.historyList = (r.data.data || []).filter(
            (it) => String(it.entry_type || "").toLowerCase() !== "progress",
          );
    ...
    async ensureUGLoadedPreferBackend(
      table = this.query.selectedGradeTable || "university_grades",
    ) {
      // 先清空本地缓存，避免在加载过程中显示旧数据
      this.csvUG = [];
      try {
        const ug = await axios.get("/api/analysis/table-data", {
          params: { table, page: 1, page_size: 10000 },
        });
        if (
          ug.data?.status === "success" &&
          Array.isArray(ug.data.data) &&
          ug.data.data.length
        ) {
    ...
    buildDetailFromCsv(studentId) {
      ...
      if (grades) {
        ...
        grades.first_calculus_score =
          grades.first_calculus_score != null
            ? toNum(grades.first_calculus_score)
            : toNum(grades.calculus_score);
        grades.second_calculus_score =
          grades.second_calculus_score != null
            ? toNum(grades.second_calculus_score)
            : null;
        grades.third_calculus_score =
          grades.third_calculus_score != null
            ? toNum(grades.third_calculus_score)
            : null;
        const parts = [
          grades.first_calculus_score,
          grades.second_calculus_score,
        ...
        grades.calculus_avg_score = parts.length
          ? Number((parts.reduce((a, b) => a + b, 0) / parts.length).toFixed(2))
          : (toNum(grades.total_score) ?? toNum(grades.calculus_score));
      }
      const percentiles = {};
      if (grades?.calculus_avg_score != null) {
        const pool = this.csvUG
          .map((r) => {
            const a = [
              this.toNum(r.first_calculus_score),
              this.toNum(r.second_calculus_score),
              this.toNum(r.third_calculus_score),
            ].filter((x) => x != null);
            return a.length
              ? a.reduce((p, c) => p + c, 0) / a.length
              : (this.toNum(r.total_score) ?? this.toNum(r.calculus_score));
          })
          .filter((x) => x != null);
        const v = grades.calculus_avg_score;
        percentiles.calculus_avg_score = pool.length
          ? (pool.filter((x) => x <= v).length / pool.length) * 100
          : null;
      }
      const factors = [];
      if (ug) {
    ...
    computeClassStats() {
      ...
      const toNum = this.toNum;
      const nums = this.csvUG
        .map((r) => {
          const parts = [
            toNum(r.first_calculus_score),
            toNum(r.second_calculus_score),
            toNum(r.third_calculus_score),
          ].filter((v) => v != null);
          return parts.length
            ? parts.reduce((a, b) => a + b, 0) / parts.length
            : (toNum(r.calculus_avg_score) ??
                toNum(r.total_score) ??
                toNum(r.calculus_score));
        })
        .filter((v) => v != null);
      const hoursArr = this.csvUG
        .map((r) => toNum(r.study_hours))
        .filter((v) => v != null);
      const firstArr = this.csvUG
        .map((r) => toNum(r.first_calculus_score))
        .filter((v) => v != null);
      const secondArr = this.csvUG
        .map((r) => toNum(r.second_calculus_score))
        .filter((v) => v != null);
      const thirdArr = this.csvUG
        .map((r) => toNum(r.third_calculus_score))
        .filter((v) => v != null);
      const avg = (arr) =>
    ...
    computeTrendFeedback() {
      ...
      const avgScore = this.toNum(
        g.calculus_avg_score ?? g.total_score ?? g.calculus_score,
      );
      ...
      if (attempts.length >= 2) {
        const last = attempts[attempts.length - 1],
          prev = attempts[attempts.length - 2];
        predicted = Math.min(100, Math.max(20, last + 0.8 * (last - prev)));
      }
      const inc12 = second != null && first != null ? second - first : null;
      const inc23 = third != null && second != null ? third - second : null;
      const steadilyUp =
        (inc12 != null && inc23 != null && inc12 >= 2 && inc23 >= 2) ||
        (attempts.length >= 2 &&
          attempts[attempts.length - 1] - attempts[0] >= 4 &&
          Math.max(...attempts) - Math.min(...attempts) <= 12);
      const volatile =
        attempts.length >= 2 &&
        Math.max(...attempts) - Math.min(...attempts) >= 15;
      if (
        steadilyUp &&
        classAvg != null &&
        predicted != null &&
        ...
        trendTexts.push(
          "你的成绩持续进步，学习状态良好，预测后续能保持优势，建议继续维持当前学习节奏。",
        );
      else if (
        volatile &&
        classAvg != null &&
        ...
        trendTexts.push(
          "成绩稳定性不足，可能受知识点掌握不扎实或学习方法影响，需重点关注波动原因，针对性调整。",
        );
      else if (predicted != null && classAvg != null)
        trendTexts.push(
          `当前预测分约为 ${predicted.toFixed(1)}，班级均分约为 ${classAvg.toFixed(1)}。`,
        );
    ...
    computeWeaknessFeedback() {
      ...
      let weakTexts = [],
        belowCnt = 0;
      if (first != null && attemptAvg.first != null && first < attemptAvg.first)
        belowCnt++;
      if (
        second != null &&
        attemptAvg.second != null &&
        second < attemptAvg.second
      )
        belowCnt++;
      if (third != null && attemptAvg.third != null && third < attemptAvg.third)
        belowCnt++;
      if (belowCnt >= 3)
        weakTexts.push(
          "高数成绩连续3次低于班级平均，且预测分仍处于下游，是你的薄弱科目，需优先分配更多学习时间。",
        );
      if (avgScore != null && classAvg != null && avgScore < classAvg - 5)
        weakTexts.push(
          `你的高数平均分 ${avgScore.toFixed(1)} 低于班级均分 ${classAvg.toFixed(1)}，建议优先补强基础知识点。`,
        );
    ...
    computeSuggestions() {
      ...
      const val = (name) => {
        const it = f.find((x) => x.name === name);
        return it ? this.toNum(it.value) : null;
      };
      let sugg = [];
      if (hours != null && avgHours != null && hours < avgHours) {
        ...
        sugg.push(
          `每周学习时长仅 ${hours.toFixed(1)} 小时，低于班级平均 ${avgHours.toFixed(1)} 小时，建议每天增加 ${incPerDay} 小时专项练习。`,
        );
      }
      if (val("出勤次数") != null && val("出勤次数") < 70)
        sugg.push(
          "出勤次数偏低，建议保证按时到课并积极参与课堂，提升学习效率。",
        );
      if (val("作业分数") != null && val("作业分数") < 75)
        sugg.push("作业得分偏低，建议规范书写、按时完成，并针对错题及时复盘。");
      if (val("刷题数") != null && val("刷题数") < 30)
        sugg.push("刷题数量不足，建议增加针对性训练，优先突破薄弱题型。");
      if (!sugg.length)
        sugg.push("维持当前学习节奏与方法，可尝试挑战更高难度题目以巩固优势。");
