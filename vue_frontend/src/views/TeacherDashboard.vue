  ...
  <div class="teacher-dashboard">
    <el-card class="main-card">
      ...
      <el-form :inline="true" class="query-form">
        ...
        <el-form-item>
          <el-button type="primary" :loading="loading" @click="loadPortrait"
            >生成画像</el-button
          >
        </el-form-item>
      </el-form>
      ...
      <div v-else>
        ...
        <el-row :gutter="16" class="kpi-row">
          <el-col :span="10">
            <el-card class="section-card" shadow="hover">
              <template #header
                ><div class="section-header">基础信息</div></template
              >
              <el-descriptions :column="2" border size="small">
                <el-descriptions-item label="ID">{{
                  portrait.profile?.student_id || "—"
                }}</el-descriptions-item>
                <el-descriptions-item label="学号">{{
                  portrait.profile?.student_no || "—"
                }}</el-descriptions-item>
                <el-descriptions-item label="姓名">{{
                  portrait.profile?.name || "—"
                }}</el-descriptions-item>
                <el-descriptions-item label="班级"
                  >{{ portrait.profile?.grade || "—" }}
                  {{ portrait.profile?.class || "" }}</el-descriptions-item
                >
              </el-descriptions>
            </el-card>
          </el-col>
          ...
          <el-col :span="14">
            <el-card class="section-card" shadow="hover">
              <template #header
                ><div class="section-header">成绩预测</div></template
              >
              <div class="pred-card" :class="{ center: !hasTrendSide }">
                <div class="kpi-main">
                  <div class="tagline">
                    <el-tag size="small" type="success">趋势外推</el-tag>
                  </div>
                  <div class="pred-value">
                    {{ formatScore(portrait.predictions?.total?.value) }}
                  </div>
                </div>
                <div v-if="hasTrendSide" class="kpi-side">
                  <div class="kpi-title">近三次成绩</div>
                  <div class="chips">
                    <span
                      v-for="(t, i) in portrait.trend || []"
                      :key="i"
                      class="chip"
                    >
                  ...
                  <div v-if="trendText" class="mini-desc">{{ trendText }}</div>
                </div>
              </div>
            </el-card>
          </el-col>
        ...
        <el-row :gutter="16">
          <el-col :span="24">
            <el-card class="section-card" shadow="hover">
              <template #header
                ><div class="section-header">薄弱知识点</div></template
              >
              <div
                v-if="
                  (analysisFb?.weaknesses && analysisFb.weaknesses.length) ||
                  portrait.weak_points?.length
                "
              >
                <ul class="list">
                  <li
                    v-for="(w, i) in analysisFb?.weaknesses?.length
                      ? analysisFb.weaknesses
                      : portrait.weak_points"
                    :key="i"
                  >
                    {{ w }}
                  </li>
        ...
        <el-row :gutter="16">
          <el-col :span="24">
            <el-card class="section-card" shadow="hover">
              <template #header
                ><div class="section-header">反馈记录</div></template
              >
        ...
        <el-card class="section-card" shadow="hover">
          <template #header
            ><div class="section-header">关联建议</div></template
          >
          <div v-if="recommendedDisplay.length">
            <el-timeline>
              <el-timeline-item
                v-for="(s, i) in recommendedDisplay"
                :key="i"
                :timestamp="`建议 ${i + 1}`"
                type="primary"
              >
...
export default {
  ...
  computed: {
    hasTrendSide() {
      const arr = Array.isArray(this.portrait?.trend)
        ? this.portrait.trend
        : [];
    ...
    recommendedDisplay() {
      const a = Array.isArray(this.portrait?.recommendations)
        ? this.portrait.recommendations
        : [];
      const b = Array.isArray(this.analysisFb?.suggestions)
        ? this.analysisFb.suggestions
        : [];
  ...
  methods: {
    ...
    async onGradeTableChange(table) {
      ...
      try {
        await this.ensureStudentsLoaded();
      } catch {}
    },
    ...
    renderHistoryText(it) {
      ...
      try {
        ...
        const w = Array.isArray(p.weaknessFeedback)
          ? p.weaknessFeedback
          : Array.isArray(p.weaknesses)
            ? p.weaknesses
            : [];
        const s = Array.isArray(p.suggestionFeedback)
      ...
      try {
        ...
        try {
          ...
          for (let i = 1; i < lines.length; i++) {
            ...
            const o = {};
            header.forEach((h, idx) => (o[h] = parts[idx] ?? ""));
            arr.push(o);
          }
    ...
    async loadPortrait() {
      if (!this.query.studentId) {
        this.$message?.warning?.("请选择学生或输入ID");
        return;
      }
      this.loading = true;
      this.portrait = null;
      ...
      try {
        ...
        if (this.query.selectedGradeTable)
          params.table = this.query.selectedGradeTable;
        const res = await axios.get("/api/teacher/student-portrait", {
          params,
        });
        if (res.data?.status === "success") {
          ...
          try {
            ...
            if (this.query.selectedGradeTable)
              hparams.table = this.query.selectedGradeTable;
            const h = await axios.get(
              "/api/analysis/student-feedback/history",
              { params: hparams },
            );
            this.historyList =
              h.data?.status === "success" && Array.isArray(h.data.data)
                ? h.data.data
                : [];
          } catch (_) {
            this.historyList = [];
          ...
          try {
            ...
            if (this.query.selectedGradeTable)
              fbparams.table = this.query.selectedGradeTable;
            const fb = await axios.get("/api/analysis/student-feedback", {
              params: fbparams,
            });
        ...
        console.error(e);
        this.$message?.error?.(
          e.response?.data?.message || e.message || "生成画像失败",
        );
      } finally {
        this.loading = false;
      }
