...
print(
    f"[JWT] [auth.py] JWT_SECRET已加载: {'已设置' if not is_default else '使用默认值'} (预览: {secret_preview})"
)
...
def authenticate_teacher(username: str, password: str):
    ...
    is_default = JWT_SECRET == "default_secret_key_change_in_production"
    print(
        f"[JWT] 生成token，使用JWT_SECRET: {'已设置' if not is_default else '默认值'} (预览: {secret_preview})"
    )
    token = jwt.encode(payload, JWT_SECRET, algorithm=JWT_ALG)
    if isinstance(token, bytes):
        token = token.decode("utf-8")
    print(
        f"[OK] Token生成成功: {token[:20]}... (长度: {len(token)}, 过期时间: {now + JWT_EXPIRE})"
    )
    return {
        "token": token,
        "teacher": {
...
def verify_token(token: str):
    try:
        ...
        if not token or "." not in token or token.count(".") != 2:
            print(
                f"[ERR] Token格式错误: token存在={bool(token)}, 包含点={'.' in token if token else False}, 点数量={token.count('.') if token else 0}"
            )
        ...
        is_default = JWT_SECRET == "default_secret_key_change_in_production"
        print(
            f"[VERIFY] 开始验证token，使用JWT_SECRET: {'已设置' if not is_default else '默认值'} (预览: {secret_preview})"
        )
        ...
        print("   可能原因: JWT_SECRET不匹配或token格式错误")
