...
class DataCollector:
    ...
    def _scan_and_schedule_jobs(self):
        try:
            ...
            rows = (
                fetch_all(
                    "SELECT id, name, type, config, active FROM data_sources WHERE active=1"
                )
                or []
            )
            configured_ids = set()
    ...
    def _collect_once(self, source_id: int, cfg: Dict[str, Any], source_name: str = ""):
        try:
            ...
            if key_col:
                ...
                st = fetch_one(
                    "SELECT last_max_id FROM data_sync_state WHERE source_id=%s",
                    [source_id],
                )
                ...
                if last is None or max_id > last:
                    ...
                    try:
                        cnt_row = fetch_one(
                            f"SELECT COUNT(1) AS cnt FROM {table} WHERE {key_col} > %s",
                            [last or 0],
                        )
                        delta_rows = (
                            int(cnt_row.get("cnt") or 0)
                            if cnt_row
                            else (max_id - (last or 0))
                        )
                    except Exception:
                    ...
                    if st:
                        execute_query(
                            "UPDATE data_sync_state SET last_max_id=%s, table_name=%s WHERE source_id=%s",
                            [max_id, table, source_id],
                        )
                    else:
                        execute_query(
                            "INSERT INTO data_sync_state (source_id, table_name, last_max_id) VALUES (%s,%s,%s)",
                            [source_id, table, max_id],
                        )
            elif upd_col:
                row = fetch_one(f"SELECT MAX({upd_col}) AS max_ts FROM {table}")
                max_ts = row.get("max_ts") if row else None
                st = fetch_one(
                    "SELECT last_max_updated FROM data_sync_state WHERE source_id=%s",
                    [source_id],
                )
                ...
                if max_ts and (not last_ts or str(max_ts) > str(last_ts)):
                    ...
                    try:
                        cnt_row = fetch_one(
                            f"SELECT COUNT(1) AS cnt FROM {table} WHERE {upd_col} > %s",
                            [last_ts or "1970-01-01"],
                        )
                    ...
                    if st:
                        execute_query(
                            "UPDATE data_sync_state SET last_max_updated=%s, table_name=%s WHERE source_id=%s",
                            [max_ts, table, source_id],
                        )
                    else:
                        execute_query(
                            "INSERT INTO data_sync_state (source_id, table_name, last_max_updated) VALUES (%s,%s,%s)",
                            [source_id, table, max_ts],
                        )
            else:
            ...
            if has_new:
                ...
                try:
                    execute_query(
                        "UPDATE data_sources SET last_collection=NOW() WHERE id=%s",
                        [source_id],
                    )
                except Exception:
                    pass
