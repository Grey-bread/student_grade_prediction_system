...
class PredictionService:
    ...
    def generate_visualizations(self, y_test, y_pred, feature_importance=None):
        ...
        if feature_importance is not None:
            ...
            plt.savefig(buf, format="png", bbox_inches="tight")
            buf.seek(0)
            visualizations["feature_importance"] = base64.b64encode(
                buf.getvalue()
            ).decode()
            plt.close()
    ...
    def train_predict(
        ...
        if target_col is None:
            ...
            if candidates:
                ...
                if not nums:
                    raise ValueError(
                        "无法识别目标列，请确保数据包含成绩列（如“总成绩/总分/分数”等）或显式指定 targetColumn"
                    )
                target_col = nums[-1]
        ...
        # 模型训练和选择
        best_model, model_results, best_params = self.model_selector.select_best_model(
            X_train, y_train
        )
        ...
        # 获取特征重要性
        feature_importance = self.model_selector.get_feature_importance(
