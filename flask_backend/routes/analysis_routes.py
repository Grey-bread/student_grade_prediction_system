    ...
    try:
        ...
        for table in tables:
            ...
            for c in cols:
                ...
                if any(subj in c_str for subj in subjects) and any(
                    k in c_str for k in ["score", "avg", "grade"]
                ):
                    hit_cols.append(c)
                    continue
        ...
        return (
            jsonify(
                {"status": "success", "tables": matched_tables, "details": details}
            ),
            200,
        )
...
def ug_score_by_effort_bucket():
    ...
    try:
        ...
        if df is None or df.empty:
            return (
                jsonify({"status": "success", "labels": [], "avg": [], "count": []}),
                200,
            )
    ...
    try:
            ...
            if not target:
                try:
                    ...
                    for c in ug.columns:
                        ...
                        if any(
                            k in low
                            for k in ["score", "grade", "total", "final", "gpa"]
                        ):
        ...
        if sid:
            try:
                ...
                if st is not None and not st.empty and "student_id" in st.columns:
                    ...
                    if not sdf.empty:
                        stu_grade = (
                            str(sdf.iloc[0].get("grade"))
                            if "grade" in sdf.columns
                            else None
                        )
                        stu_class = (
                            str(sdf.iloc[0].get("class"))
                            if "class" in sdf.columns
                            else None
                        )
                        cols_to_merge = [
                            c
                            for c in ["student_id", "grade", "class"]
                            if c in st.columns
                        ]
                        if cols_to_merge:
                            ugm = ug.merge(
                                st[cols_to_merge], on="student_id", how="left"
                            )
                            if (
                                stu_grade
                                and "grade" in ugm.columns
                                and ugm["grade"].notna().any()
                            ):
                                ug = ugm[ugm["grade"].astype(str) == stu_grade]
                            elif (
                                stu_class
                                and "class" in ugm.columns
                                and ugm["class"].notna().any()
                            ):
            ...
            if missing_all:
                try:
                    ...
                    def is_factor_col(c: str):
                        ...
                        if c == target:
                            return False
                        if cl.endswith("_id") or cl in ("id", "student_id"):
                            return False
                        return True
        ...
        for col in factors:
            ...
            try:
                ...
                data_map = {
                    str(k): float(v) if pd.notna(v) else 0.0 for k, v in grp.items()
                }
...
def _normalize_university_grades_df(df: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        if "first_calculus_score" not in cols and "calculus_score" in cols:
            df["first_calculus_score"] = pd.to_numeric(
                df["calculus_score"], errors="coerce"
            )
        if "second_calculus_score" not in cols:
            df["second_calculus_score"] = (
                pd.to_numeric(df.get("second_calculus_score"), errors="coerce")
                if "second_calculus_score" in cols
                else np.nan
            )
        if "third_calculus_score" not in cols:
            df["third_calculus_score"] = (
                pd.to_numeric(df.get("third_calculus_score"), errors="coerce")
                if "third_calculus_score" in cols
                else np.nan
            )
        ...
        if "calculus_avg_score" not in cols:
            ...
            if df["calculus_avg_score"].isna().all():
                if "total_score" in df.columns:
                    df["calculus_avg_score"] = pd.to_numeric(
...
def student_feedback():
    ...
    try:
        ...
        if (
            ...
            and not courses_df.empty
            and "course_id" in courses_df.columns
        ):
            for _, r in courses_df.iterrows():
                try:
                    course_name_map[int(r["course_id"])] = str(
                        r.get("course_name") or f"课程{r['course_id']}"
                    )
                except Exception:
                    continue
        ...
        if hg_df is not None and not hg_df.empty:
            ...
            if not sdf.empty:
                ...
                if latest_row is not None and "total_score" in latest_row:
                    try:
                        overview["latest_total_score"] = round(
                            float(latest_row["total_score"]), 2
                        )
                    except Exception:
                        pass
                # 按课程汇总
                        ...
                        overview["by_course"].append(
                            {
                                "course_id": cid,
                                "course_name": (
                                    course_name_map.get(cid, f"课程{cid}")
                                    if cid
                                    else "未知课程"
                                ),
                                "midterm_score": (
                                    round(float(r["midterm_score"]), 2)
                                    if pd.notna(r.get("midterm_score"))
                                    else None
                                ),
                                "final_score": (
                                    round(float(r["final_score"]), 2)
                                    if pd.notna(r.get("final_score"))
                                    else None
                                ),
                                "usual_score": (
                                    round(float(r["usual_score"]), 2)
                                    if pd.notna(r.get("usual_score"))
                                    else None
                                ),
                                "total_score": (
        ...
        if exam_df is not None and not exam_df.empty:
            ...
            if not edf.empty:
                ...
                if "exam_date" in edf.columns:
                    edf["exam_date_parsed"] = pd.to_datetime(
                        edf["exam_date"], errors="coerce"
                    )
                else:
        ...
        # 课堂表现：学生均值 vs 全部均值
        perf_summary = {"student_avg": {}, "class_avg": {}}
        perf_keys = [
            "attendance_score",
            "participation_score",
            "homework_score",
        ...
        if perf_df is not None and not perf_df.empty:
            ...
            # 学生数据
            pdf = (
                perf_df[perf_df["student_id"].astype(str) == str(student_id)]
                if "student_id" in perf_df.columns
                else pd.DataFrame()
            )
            # 课程过滤
        ...
        traceback.print_exc(file=sys.stdout)
        return (
            jsonify({"status": "error", "message": f"生成学生反馈失败: {str(e)}"}),
            500,
        )
...
def save_student_feedback():
    ...
    try:
        ...
        try:
            ...
            try:
                execute_query(
                    "DELETE FROM student_feedbacks WHERE student_id=%s", (student_id,)
                )
            except Exception:
                pass
            execute_query(
                "INSERT INTO student_feedbacks (student_id, content) VALUES (%s, %s)",
                (student_id, content),
            )
        # 追加一条历史（不影响主流程，失败忽略）
        try:
            ...
            if isinstance(_data, dict):
                ...
                if isinstance(w, list):
                    weak = [str(x) for x in w if x][:2]
                if isinstance(s, list):
                    sugg = [str(x) for x in s if x][:2]
...
def get_saved_student_feedback():
    ...
    try:
        ...
        row = fetch_one(
            "SELECT content FROM student_feedbacks WHERE student_id=%s", (student_id,)
        )
        if not row:
        ...
        traceback.print_exc(file=sys.stdout)
        return (
            jsonify({"status": "error", "message": f"读取保存反馈失败: {str(e)}"}),
            500,
        )
...
def get_distinct_values():
    ...
    try:
        ...
        if not table_name or not column:
            return (
                jsonify({"status": "error", "message": "缺少参数: table 或 column"}),
                400,
            )
        ...
        try:
            rows = fetch_all(
                f"SELECT DISTINCT {column} AS val FROM {table_name} WHERE {column} IS NOT NULL LIMIT 2000"
            )
            if rows:
                for r in rows:
        ...
        if not values:
            ...
            if df is not None and not df.empty and column in df.columns:
                ...
                try:
                    ...
                    values = [
                        str(v) for v in list(dict.fromkeys(ser.astype(str).tolist()))
                    ]
    ...
    try:
        ...
        try:
            ...
            if time_column in ["exam_date"]:
                try:
                    grouped_data[time_column] = pd.to_datetime(
                        grouped_data[time_column]
                    )
            ...
            for col in numeric_columns:
                if "score" in col.lower() and not any(
                    keyword in col.lower() for keyword in ["id", "编号"]
                ):
                ...
                series.append(
                    {
                        ...
                        "data": [
                            round(float(val), 1)
                            for val in grouped_data[actual_score_col].tolist()
                        ],
                    }
                )
                legend.append("实际成绩")
            ...
            if len(series) > 0:
                ...
                # 生成基于实际数据的预测
                predicted_data = [
                    round(val + np.random.uniform(-2, 2), 1) for val in actual_data
                ]
                series.append(
                    {"name": "预测成绩", "type": "line", "data": predicted_data}
                )
                legend.append("预测成绩")
            ...
            if not series:
                ...
                for col in numeric_columns:
                    if not any(
                        ...
                        for keyword in ["id", "编号", "number", "num"]
                    ):
                        ...
                        series.append(
                            {
                                ...
                                "data": [
                                    round(float(val), 1)
                                    for val in grouped_data[col].tolist()
                                ],
                            }
                        )
            ...
            jsonify(
                {
                    ...
                    "legend": ["实际成绩", "预测成绩"],
                    "series": [
                        {"name": "实际成绩", "type": "line", "data": [78, 82, 85, 88]},
                        {"name": "预测成绩", "type": "line", "data": [80, 83, 86, 89]},
                    ],
                }
            ),
            200,
        ...
        traceback.print_exc(file=sys.stdout)
        return (
            jsonify({"status": "error", "message": f"获取学生趋势数据失败: {str(e)}"}),
            500,
        )
...
def get_model_accuracy():
    ...
    try:
        ...
        # 返回模拟的模型评估指标
        metrics = [{"value": [85, 90, 88, 95], "name": f"{model} 模型评估指标"}]
        ...
        return (
            jsonify(
                {"status": "error", "message": f"获取模型准确度数据失败: {str(e)}"}
            ),
            500,
        )
...
def get_student_progress():
    ...
    try:
        ...
        return (
            jsonify({"status": "success", "labels": labels, "progress": progress}),
            200,
        )
        ...
        return (
            jsonify(
                {"status": "error", "message": f"获取学生进步情况数据失败: {str(e)}"}
            ),
            500,
        )
...
def get_numeric_columns(df, table_name):
    ...
    print(
        f"数值列识别完成，最终识别到 {len(numeric_columns)} 个数值列: {numeric_columns}"
    )
...
def get_table_data(table_name):
    ...
    # 尝试从缓存获取数据
    if (
        "processed_data" in global_data
        and global_data.get("current_table") == table_name
    ):
        print("从缓存获取已处理数据")
        try:
        ...
        "current_data" in global_data and global_data.get("current_table") == table_name
    ):
        print("从缓存获取原始数据")
        try:
    ...
    if df is not None:
        ...
        numeric_columns = structure.get("score_columns", []) + structure.get(
            "ranking_columns", []
        )
        ...
        for col in numeric_columns[1:]:
            ...
            sample_data[col] = (
                correlation * sample_data[numeric_columns[0]]
                + (1 - correlation) * np.random.normal(70, 10, size=50)
                + noise
            ).astype(int)
...
def get_statistics():
    ...
    try:
        ...
        if df is None:
            return (
                jsonify(
                    {"status": "error", "message": f"没有找到{table_name}表的数据"}
                ),
                404,
            )
        ...
        traceback.print_exc(file=sys.stdout)
        return (
            jsonify({"status": "error", "message": f"获取统计数据失败: {str(e)}"}),
            500,
        )
...
def preprocess_data():
    ...
    try:
        ...
        df = get_table_data(table_name)
        if df is None:
            return jsonify({"status": "error", "message": "没有数据可供处理"}), 404
        ...
        # 预处理数据
        processed_df, encoders = preprocess_df(
            df, missing_value_strategy, outlier_strategy
        )
...
def feature_engineering():
    ...
    try:
        ...
        if df is None:
            ...
            return (
                jsonify(
                    {"status": "error", "message": f"没有找到表 {table_name} 的数据"}
                ),
                404,
            )
        ...
        if len(numeric_columns) >= 2:
            try:
                ...
                # 使用默认值继续
                features = [
                    get_friendly_column_name(col) for col in numeric_columns[:4]
                ]  # 限制最多4个特征
...
def get_class_trends():
    ...
    try:
        ...
        try:
            ...
            for col in numeric_columns:
                ...
                if not any(
                    ...
                    series.append({"name": series_name, "type": "line", "data": data})
        ...
        traceback.print_exc(file=sys.stdout)
        return (
            jsonify({"status": "error", "message": f"获取班级趋势数据失败: {str(e)}"}),
            500,
        )
...
def get_subject_comparison():
    ...
    try:
        ...
        table_name = request.args.get(
            "table", global_data.get("current_table", "students")
        )
        ...
        # 处理剩余的数值列
        remaining_columns = [
            col for col in numeric_columns if col not in processed_columns
        ]
        ...
        for col in remaining_columns:
            if not any(
                keyword in col.lower()
                for keyword in ["id", "编号", "number", "num", "ranking", "排名"]
            ):
            ...
            jsonify(
                {
                    ...
                    "legend": ["平均分"],
                    "series": [{"name": "平均分", "type": "bar", "data": scores}],
                }
            ),
            200,
        )
        ...
        traceback.print_exc(file=sys.stdout)
        return (
            jsonify({"status": "error", "message": f"获取学科对比数据失败: {str(e)}"}),
            500,
        )
...
def get_correlation():
    ...
    try:
        ...
        return (
            jsonify({"status": "success", "features": features, "data": data_points}),
            200,
        )
...
def get_grade_distribution():
    try:
        ...
        if not score_level_column or score_level_column not in df.columns:
            ...
            return (
                jsonify(
                    {"status": "success", "data": data, "total": 0, "table": table_name}
                ),
                200,
            )
        ...
        if "student_id" in df.columns:
            ...
            if len(student_levels) == 0:
                return (
                    jsonify(
                        {"status": "error", "message": "没有有效的学生成绩等级数据"}
                    ),
                    404,
                )
            ...
            print(
                f"[饼图] 按学生统计，共{total_students}个学生，等级分布: {level_counts.to_dict()}"
            )
            ...
            if len(score_levels) == 0:
                return (
                    jsonify({"status": "error", "message": "没有有效的成绩等级数据"}),
                    404,
                )
            level_counts = score_levels.value_counts()
            print(
                f"[饼图] 所有记录统计，共{len(score_levels)}条记录，等级分布: {level_counts.to_dict()}"
            )
        ...
        for level in all_levels:
            ...
            data.append({"name": level_names[level], "value": count})

        total_count = int(level_counts.sum()) if len(level_counts) > 0 else 0
        count_type = "个学生" if "student_id" in df.columns else "条记录"
        print(
            f"[饼图] 等级分布 - A:{level_counts.get('A', 0)}, B:{level_counts.get('B', 0)}, C:{level_counts.get('C', 0)}, D:{level_counts.get('D', 0)}, E:{level_counts.get('E', 0)}, 总计:{total_count}{count_type}"
        )
...
def get_score_band_distribution():
    ...
    try:
        ...
        if df is None or df.empty:
            return (
                jsonify(
                    {"status": "success", "data": [], "total": 0, "table": table_name}
                ),
                200,
            )
        # Auto-select column
...
def get_radar_data():
    ...
    try:
        ...
        df = get_table_data(table_name)
        if df is None or df.empty:
            return jsonify({"status": "error", "message": "无法获取数据"}), 404
            ...
            try:
                ...
                if student_id and "student_id" in df.columns:
                    ...
                    if not student_df.empty:
                        ...
                        student_data.append(
                            round(student_val, 2) if not np.isnan(student_val) else 0
                        )
                    else:
                ...
                jsonify(
                    {
                        ...
                        "series": [{"name": "暂无有效数据", "value": [0, 0, 0]}],
                    }
                ),
                200,
            )
        ...
        # 构建返回数据
        series_data = [{"name": "班级平均", "value": class_avg}]

        if student_id and len(student_data) > 0 and len(student_data) == len(class_avg):
            series_data.append({"name": f"学生{student_id}", "value": student_data})

        return (
            jsonify(
                {"status": "success", "indicator": indicators, "series": series_data}
            ),
            200,
        )
        ...
        with zipfile.ZipFile(
            ...
            try:
                ...
                if not numeric_df.empty:
                    ...
                    zf.writestr(
                        "analysis/correlation_info.txt", "无数值列可用于相关性分析"
                    )
            except Exception as e:
            ...
            try:
                ...
                if isinstance(resp, tuple):
                    ...
                    radar_json = (
                        resp_data.get_json() if hasattr(resp_data, "get_json") else None
                    )
                else:
...
def create_record(table_name):
    ...
    try:
            ...
            if not sid:
                ...
                if st is not None and not st.empty:
                    if data.get("student_no"):
                        srow = st[
                            st["student_no"].astype(str) == str(data["student_no"])
                        ]
                        if len(srow) == 1:
                            ...
                            return (
                                jsonify(
                                    {
                                        "status": "error",
                                        "message": "未找到该学号对应的学生，请先在学生表中创建或检查学号",
                                    }
                                ),
                                400,
                            )
                        ...
                        if len(srow) == 1:
                            ...
                            return (
                                jsonify(
                                    {
                                        "status": "error",
                                        "message": "存在同名学生，请提供学号 student_no 以唯一定位",
                                    }
                                ),
                                400,
                            )
        ...
        missing = [
            r for r in required if (r not in data or data.get(r) in (None, "", []))
        ]
        if missing:
            return (
                jsonify(
        ...
        if table_name == "university_grades" and "student_id" in data:
            try:
                ...
                if (
                    st is None
                    or st.empty
                    or "student_id" not in st.columns
                    or not (
                        st["student_id"].astype(str) == str(data["student_id"])
                    ).any()
                ):
                    return (
                        jsonify(
                            {
                                "status": "error",
                                "message": "student_id 不存在于学生表，请确认后再提交",
                            }
                        ),
                        400,
                    )
        ...
        if table_name == "university_grades":
            ...
            if "calculus_avg_score" not in data:
                parts = [
                    to_num(data.get("first_calculus_score")),
                    to_num(data.get("second_calculus_score")),
                    to_num(data.get("third_calculus_score")),
                ]
        ...
        # 清空缓存，强制重新加载
        if (
            "current_table" in global_data
            and global_data["current_table"] == table_name
        ):
            global_data.clear()
    ...
    try:
        ...
        if table_name == "university_grades":
            try:
                ...
                if row:
                    s1 = data.get(
                        "first_calculus_score", row.get("first_calculus_score")
                    )
                    s2 = data.get(
                        "second_calculus_score", row.get("second_calculus_score")
                    )
                    s3 = data.get(
                        "third_calculus_score", row.get("third_calculus_score")
                    )
...
def get_student_courses():
    ...
    try:
        ...
        if students_df is not None and not students_df.empty:
            if student_id:
                srow = students_df[
                    students_df["student_id"].astype(str) == str(student_id)
                ]
                if not srow.empty:
                    r = srow.iloc[0]
                    resolved_student = {
        ...
        traceback.print_exc(file=sys.stdout)
        return (
            jsonify({"status": "error", "message": f"获取学生课程失败: {str(e)}"}),
            500,
        )
...
def sync_data_sources_with_db():
    ...
    try:
        ...
        for r in rows:
            ...
            try:
                cfg = (
                    json.loads(cfg_raw) if str(cfg_raw).strip().startswith("{") else {}
                )
            except Exception:
                cfg = {}
            t = cfg.get("table")
        ...
        for t in sorted(actual):
            if t not in ds_by_table:
                ...
                if (r.get("name") or "") != expect_name:
                    try:
                        execute_query(
                            "UPDATE data_sources SET name=%s WHERE id=%s",
                            [expect_name, r.get("id")],
                        )
                    except Exception:
                        pass
    except Exception:
    ...
    try:
                ...
                data.append(
                    {
                        ...
                        "uploadTime": (
                            str(r.get("uploaded_at")) if r.get("uploaded_at") else None
                        ),
        ...
        for f in files:
            try:
                ...
                try:
                    if fname.lower().endswith(".csv"):
                        ...
                        try:
                            ...
                            raise RuntimeError(
                                "读取 Excel 需要安装 openpyxl，请安装后重试"
                            ) from ie
                    else:
                        df = pd.read_csv(str(target_path))
                except Exception as read_err:
                    execute_query(
                        "UPDATE upload_history SET status='failed', message=%s WHERE id=%s",
                        [f"读取文件失败: {read_err}", up_id],
                    )
                    saved.append(
                        {
                            "filename": fname,
                            "uploadTime": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                            "fileSize": format_file_size_safe(
                                os.path.getsize(target_path)
                            ),
                            "status": "failed",
                        }
                ...
                if df is None or df.empty:
                    execute_query(
                        "UPDATE upload_history SET message=%s WHERE id=%s",
                        ["文件无数据，未创建数据表", up_id],
                    )
                else:
                    # 基于原始文件名推断表名
                    ...
                    for idx, col in enumerate(list(df.columns)):
                        ...
                        if (
                            (not base_name)
                            or (base_name.strip() == "")
                            or (base_name.strip().lower() in ("nan", "none", "null"))
                        ):
                            base_name = f"列{idx+1}"
                        # 去重：如重复则追加后缀 _2, _3...
                        unique = base_name
                    ...
                    if pk:
                        cols_sql = [
                            (
                                c.replace("`id` BIGINT", "`id` BIGINT AUTO_INCREMENT")
                                if c.startswith("`id` BIGINT")
                                else c
                            )
                            for c in cols_sql
                        ]
                    ...
                    try:
                        ...
                        saved.append(
                            {
                                "filename": fname,
                                "uploadTime": datetime.now().strftime(
                                    "%Y-%m-%d %H:%M:%S"
                                ),
                                "fileSize": format_file_size_safe(
                                    os.path.getsize(target_path)
                    ...
                    try:
                        ...
                        try:
                            ...
                            pass
                        records = df2.where(pd.notnull(df2), None).to_dict(
                            orient="records"
                        )
                        if records:
                            ...
                            cols_safe = [
                                c for c in list(df2.columns) if c in cols_exist
                            ]
                            if not cols_safe:
                            ...
                            insert_sql = (
                                ...
                                + ",".join([f"`{c}`" for c in cols_safe])
                                + f") VALUES ({placeholders})"
                            )
                            params = [
                                tuple(rec.get(c) for c in cols_safe) for rec in records
                            ]
                            try:
                                ...
                                if "Unknown column" in str(ie):
                                    ...
                                    params = [
                                        tuple(rec.get(c) for c in cols_safe)
                                        for rec in records
                                    ]
                                    execute_many(insert_sql, params)
                                else:
                        ...
                        try:
                            execute_query(
                                "DELETE FROM table_column_mapping WHERE table_name=%s",
                                [table_name],
                            )
                            mapping_rows = []
                            for idx, orig_col in enumerate(list(df.columns)):
                                ...
                                try:
                                    orig_str = (
                                        "" if pd.isna(orig_col) else str(orig_col)
                                    )
                                except Exception:
                                    orig_str = (
                                        str(orig_col) if orig_col is not None else ""
                                    )
                                stored = (
                                    ordered_new_cols[idx]
                                    if idx < len(ordered_new_cols)
                                    else f"col_{idx+1}"
                                )
                                display = orig_str or stored
                                mapping_rows.append(
                                    (table_name, idx + 1, stored, orig_str, display)
                                )
                            execute_many(
                            ...
                            try:
                                if table_name in (
                                    global_data.get("column_labels") or {}
                                ):
                        ...
                        try:
                            ...
                            exist = fetch_one(
                                "SELECT id FROM data_sources WHERE name=%s", [ds_name]
                            )
                            if not exist:
                                cols_exist = set(get_columns(table_name) or [])
                ...
                data.append(
                    {
                        ...
                        "lastCollection": (
                            str(r.get("last_collection"))
                            if r.get("last_collection")
                            else None
                        ),
                    }
                )
    ...
    try:
        ...
        if request.method == "GET":
            ...
            data = {
                ...
                "lastCollection": (
                    str(row.get("last_collection"))
                    if row.get("last_collection")
                    else None
                ),
                "createdAt": (
        ...
        if request.method == "PUT":
            ...
            if not name or not dtype:
                return (
                    jsonify({"status": "error", "message": "name 与 type 为必填"}),
                    400,
                )
...
def collect_now(source_id: int):
    ...
    try:
        ...
        row = fetch_one(
            "SELECT id, name, config FROM data_sources WHERE id=%s", [source_id]
        )
        if not row:
            return jsonify({"status": "error", "message": "数据源不存在"}), 404
        name = row.get("name")
        ...
        try:
            ...
            return (
                jsonify({"status": "error", "message": f"采集执行失败: {str(e)}"}),
                500,
            )
...
def collection_tasks_endpoint():
    ...
    try:
        ...
        if request.method == "GET":
                ...
                data.append(
                    {
                        ...
                        or (f"源#{r.get('source_id')}" if r.get("source_id") else ""),
            ...
            return (
                jsonify({"status": "success", "message": "任务已创建", "id": new_id}),
                201,
            )
        ...
        traceback.print_exc(file=sys.stdout)
        return (
            jsonify({"status": "error", "message": f"采集任务处理失败: {str(e)}"}),
            500,
        )
...
def collection_runs_endpoint():
    ...
    try:
        ...
        traceback.print_exc(file=sys.stdout)
        return (
            jsonify({"status": "error", "message": f"获取采集记录失败: {str(e)}"}),
            500,
        )
    ...
    try:
        ...
        for r in rows:
            ...
            if not cols:
                # 表不存在，跳过，仅记录
                updated.append(
                    {"id": sid, "name": r.get("name"), "action": "table_missing"}
                )
                continue
            new_cfg = dict(cfg)
            ...
            if "updated_at" in cols:
                if new_cfg.get("updated_at_column") != "updated_at" or new_cfg.get(
                    "key_column"
                ):
                ...
                if new_cfg.get("key_column") != "id" or new_cfg.get(
                    "updated_at_column"
                ):
                ...
                if new_cfg.get("key_column") != "student_id" or new_cfg.get(
                    "updated_at_column"
                ):
            ...
            if changed:
                execute_query(
                    ...
                    [json.dumps(new_cfg, ensure_ascii=False), sid],
                )
                updated.append(
                    {
                        "id": sid,
