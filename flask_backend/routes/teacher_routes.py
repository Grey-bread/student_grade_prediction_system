    ...
    try:
        ...
        try:
            ...
            if not token or not token.strip() or token.count(".") != 2:
                return (
                    jsonify({"status": "error", "message": "无效的认证令牌格式"}),
                    401,
                )
            try:
        ...
        return jsonify({"status": "success", "message": "个人信息更新成功"}), 200
...
def change_password():
    ...
    try:
        ...
        if not current_password or not new_password:
            return (
                jsonify({"status": "error", "message": "当前密码和新密码不能为空"}),
                400,
            )

        # 获取当前用户信息
        teacher = fetch_one(
            "SELECT password FROM teachers WHERE teacher_id = %s", (teacher_id,)
        )
        if not teacher:
        ...
        try:
            ...
            for record in login_records:
                if isinstance(record["login_time"], datetime.datetime):
                    record["login_time"] = record["login_time"].strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )

            return jsonify({"status": "success", "data": login_records}), 200
        except Exception:
            # 如果数据库操作失败，返回一些模拟数据
            mock_data = [
                {
...
def upload_avatar():
    try:
        ...
        if not auth_header or not auth_header.startswith("Bearer "):
            return jsonify({"status": "error", "message": "未提供有效的认证令牌"}), 401
        ...
        if "avatar" not in request.files:
            return jsonify({"status": "error", "message": "未收到上传文件"}), 400
        ...
        try:
            ...
            if "Unknown column" in str(db_err) and "avatar" in str(db_err):
                try:
                    execute_query(
                        "ALTER TABLE teachers ADD COLUMN avatar LONGTEXT NULL"
                    )
                    execute_query(update_query, (avatar_data, teacher_id))
        ...
        return (
            jsonify({"status": "error", "message": "头像上传失败", "error": str(e)}),
            500,
        )
...
def dashboard_overview():
    ...
    try:
        ...
        # 过滤本教师数据
        teacher_exam = (
            exam_df[exam_df["teacher_id"].astype(str) == tid]
            if not exam_df.empty and "teacher_id" in exam_df.columns
            else pd.DataFrame()
        )
        teacher_hist = (
            hist_df[hist_df["teacher_id"].astype(str) == tid]
            if not hist_df.empty and "teacher_id" in hist_df.columns
            else pd.DataFrame()
        )
        ...
        if (
            avg_score is None
            and not teacher_hist.empty
            and "total_score" in teacher_hist.columns
        ):
    ...
    try:
        ...
        for cid in sorted(course_ids):
            ...
            for df in [ex, hg]:
                if not df.empty:
                    sdf = (
                        df[df["course_id"].astype(str) == str(cid)]
                        if "course_id" in df.columns
                        else pd.DataFrame()
                    )
            ...
            if avg is None and not hg.empty and "total_score" in hg.columns:
                avg = _safe_mean(
                    hg[hg["course_id"].astype(str) == str(cid)]["total_score"]
                )
            # 最新考试日期
            last_date = None
            if not ex.empty and "exam_date" in ex.columns:
                ...
                if not e1.empty:
                    e1["exam_date_parsed"] = pd.to_datetime(
                        e1["exam_date"], errors="coerce"
                    )
                    if e1["exam_date_parsed"].notna().any():
                        last_date_ts = e1["exam_date_parsed"].max()
                        last_date = (
                            str(last_date_ts.date()) if pd.notna(last_date_ts) else None
                        )
        ...
        traceback.print_exc(file=sys.stdout)
        return (
            jsonify({"status": "error", "message": f"获取课程列表失败: {str(e)}"}),
            500,
        )
...
def dashboard_recent_exams():
    ...
    try:
        ...
        ex["exam_date_parsed"] = (
            pd.to_datetime(ex["exam_date"], errors="coerce")
            if "exam_date" in ex.columns
            else pd.NaT
        )
        ...
        traceback.print_exc(file=sys.stdout)
        return (
            jsonify({"status": "error", "message": f"获取最近考试失败: {str(e)}"}),
            500,
        )
...
def dashboard_score_level():
    ...
    try:
        ...
        traceback.print_exc(file=sys.stdout)
        return (
            jsonify({"status": "error", "message": f"获取等级分布失败: {str(e)}"}),
            500,
        )
...
def student_portrait():
    ...
    try:
        ...
        target_table = (
            ...
            or request.args.get("table")
            or "university_grades"
        )
        target_column = request.args.get("target_column")
        include_subjects = (
            str(request.args.get("include_subjects", "true")).lower() != "false"
        )
        ...
        try:
            print(
                f"[DEBUG student_portrait] student_id={student_id} target_table={target_table}"
            )
        except Exception:
            pass
        ...
        if (
            df_target is None or df_target.empty
        ) and target_table != "university_grades":
            # 回退到 UG（仅单科）
        ...
        if (
            ...
            and "calculus_avg_score" not in df_target.columns
        ):
            if all(
                col in df_target.columns
                for col in [
                ...
                try:
                    ...
                    df_target["calculus_avg_score"] = df_target[
                        [
                            "first_calculus_score",
                            "second_calculus_score",
                            "third_calculus_score",
        ...
        if (
            df_target is not None
            and not df_target.empty
            and chosen_target_col
            and chosen_target_col in df_target.columns
        ):
            try:
                # 预处理与建模
                ...
                if chosen_target_col not in df_proc.columns:
                    ...
                    pass
                X_all = df_proc.drop(
                    columns=[c for c in [chosen_target_col] if c in df_proc.columns]
                )
                y_all = (
                    df_proc[chosen_target_col]
                    if chosen_target_col in df_proc.columns
                    else None
                )
                if y_all is None:
                    # 无法定位目标列，跳过预测
                ...
                X_train, X_test, y_train, y_test = train_test_split(
                    X_all.values, y_all.values, test_size=0.2, random_state=42
                )
                best_model, model_results, best_params = selector.select_best_model(
                    X_train, y_train
                )
                # 在全量数据拟合
                ...
                sraw = (
                    df_target[
                        df_target["student_id"].astype(str) == str(student_id)
                    ].copy()
                ...
                if not sraw.empty:
                    ...
                    if "exam_date" in sraw.columns:
                        sraw["exam_date_parsed"] = pd.to_datetime(
                            sraw["exam_date"], errors="coerce"
                        )
        ...
        try:
            ...
            if (
                ug is None
                or ug.empty
                or "student_id" not in ug.columns
                or not all(c in ug.columns for c in ug_cols)
            ):
            ...
            if (
                ug is not None
                and not ug.empty
                and "student_id" in ug.columns
                and all(c in ug.columns for c in ug_cols)
            ):
                # 转为数值
                for c in ug_cols:
        ...
        def _trend_forecast(vals):
            try:
                ...
                y = _np.array(
                    [float(v) if v is not None else 0.0 for v in vals], dtype=float
                )
        ...
        try:
            ...
            for rr in rows:
                ...
                if not text:
                    ...
                    # 反馈类：取弱点/建议前2条组成摘要
                    ww = (
                        payload.get("weaknessFeedback")
                        or payload.get("weaknesses")
                        or []
                    )
                    ss = (
                        payload.get("suggestionFeedback")
        ...
        if ex is not None and not ex.empty and "student_id" in ex.columns:
            ...
            if not ed.empty:
                if "exam_date" in ed.columns:
                    ed["exam_date_parsed"] = pd.to_datetime(
                        ed["exam_date"], errors="coerce"
                    )
        ...
        feedback_records.sort(
            key=lambda x: (_ts(x.get("time")) if x.get("time") else pd.NaT)
        )
        ...
        if (
            ...
            and "student_id" in perf_df.columns
        ):
            ...
            cls_avg = {
                k: (
                    float(perf_df[k].mean())
                    if k in perf_df.columns and perf_df[k].notna().any()
                    else None
                )
                for k in perf_keys
            }
            stu_avg = {
                k: (
                    float(stu[k].mean())
                    if not stu.empty and k in stu.columns and stu[k].notna().any()
                    else None
                )
                for k in perf_keys
            }
            ...
            for k in perf_keys:
                ...
                if s <= c - 5:
                    weak_points.append(
                        f"{name_map.get(k,k)}低于班级平均 {round(c - s, 1)} 分"
                    )
                    if k == "attendance_score":
                        recommendations.append(
                            "教师可通过温和约定与贴心提醒，帮助学生逐步稳定作息与到课习惯；必要时与家长同频陪伴。"
                        )
                    if k == "participation_score":
                        recommendations.append(
                            "教师可用点名/小组展示等友好方式，帮助学生获得表达机会，并在课后予以鼓励。"
                        )
                    if k == "homework_score":
                        recommendations.append(
                            "教师可明确作业期望与示例，帮助学生每周用5–10分钟轻松复盘，发现亮点与改进处。"
                        )
                    if k == "behavior_score":
                        recommendations.append(
                            "教师可与学生共建轻量可行的课堂约定，帮助其获得即时而友好的反馈，必要时与班主任形成合力。"
                        )
        ...
        if td:
            if ("回落" in td) or ("下降" in td):
                add_rec(
                    "教师可先稳后进，先聚焦必考核心与高频题型，用短周期小测温和巩固，帮助学生减轻负担并稳住节奏。",
                    3.0,
                )
            if ("上扬" in td) or ("回升" in td):
                add_rec(
                    "教师可及时送上正向反馈，适度提升练习难度，安排“巩固→提升”的阶梯式练习，帮助学生保持动力。",
                    2.5,
                )
            if ("平稳" in td) or ("稳定" in td):
                add_rec(
                    "教师可引入轻挑战任务（微项目/变式题），帮助学生在稳定中寻找小幅度跃升的机会。",
                    2.5,
                )
        ...
        if pred_total is not None and chosen_target_col:
            ...
            try:
                ...
                if pv < 60:
                    add_rec(
                        "教师可帮助学生夯实基础，先达及格线；以“核心例题+类题2-3道”滚动巩固。",
                        3.0,
                    )
                elif pv < 75:
                    add_rec(
                        "教师可帮助学生围绕薄弱模块设立微目标（+5分），分周推进，并用周测校准节奏。",
                        2.5,
                    )
                elif pv >= 85:
                    add_rec(
                        "教师可帮助学生引入提升性任务（难题变式/综合题），锻炼方法迁移与表达。",
                        2.0,
                    )
            except Exception:
                pass
        ...
        try:
            for w in weak_points or []:
                ...
                if any(k in ws for k in ["出勤", "缺勤", "迟到"]):
                    add_rec(
                        "教师可与学生轻松约定到课安排，配合签到提醒与原因记录，帮助其建立稳定习惯；必要时与家长温和协作。",
                        2.5,
                    )
                if any(k in ws for k in ["参与", "互动", "发言"]):
                    add_rec(
                        "教师可用点名/小组展示与结构化发言卡降低开口门槛，帮助学生更轻松开口表达。",
                        2.5,
                    )
                if any(k in ws for k in ["作业", "完成", "拖延"]):
                    add_rec(
                        "教师可明确作业清单与标准，课初3分钟简要抽检，帮助学生及时纠错并积累亮点。",
                        2.5,
                    )
                if any(k in ws for k in ["纪律", "课堂秩序"]):
                    add_rec(
                        "教师可与学生共同约定课堂规范，记录与反馈做闭环，帮助其内化课堂规则；必要时与班主任形成合力。",
                        2.5,
                    )
        except Exception:
            pass
        ...
        try:
            ...
            saved = _fetch_one(
                "SELECT content FROM student_feedbacks WHERE student_id=%s",
                (student_id,),
            )
            if saved and saved.get("content"):
                ...
                if isinstance(saved_obj, dict):
                    # 薄弱点
                    try:
                        sv_wk = saved_obj.get("weaknessFeedback") or saved_obj.get(
                            "weaknesses"
                        )
                    ...
                    # 建议
                    try:
                        sv_sug = saved_obj.get("suggestionFeedback") or saved_obj.get(
                            "suggestions"
                        )
                        if isinstance(sv_sug, list) and sv_sug:
                            # 合并学生反馈建议（置于教师建议之后，避免覆盖），最终最多5条
                            recommendations = list(
                                dict.fromkeys(recommendations + sv_sug)
                            )[:5]
                    except Exception:
                        pass
                    # 反馈记录
        ...
        traceback.print_exc(file=sys.stdout)
        return (
            jsonify({"status": "error", "message": f"生成学生画像失败: {str(e)}"}),
            500,
        )
