...
def train_model():
    ...
    try:
        ...
        print(
            f"[TRAIN] 开始训练模型 - 目标列: {target_column}, 测试集比例: {test_size}"
        )
        ...
        if table_override:
            ...
            if df is None or df.empty:
                return (
                    jsonify(
                        {
                            "status": "error",
                            "message": f"表 {table_override} 无数据可用于训练",
                        }
                    ),
                    400,
                )
            ...
            if not rows or len(rows) == 0:
                return (
                    jsonify(
                        {"status": "error", "message": "数据库中没有可用的训练数据"}
                    ),
                    400,
                )
            ...
            return jsonify({"status": "error", "message": "暂不支持上传文件训练"}), 400

        # 检查目标列是否存在
        if target_column not in df.columns:
            available_cols = df.select_dtypes(
        ...
        print("[TRAIN] 开始模型训练...")
        result = prediction_service.train_predict(
            df, target_col=target_column, test_size=test_size
        )
...
def predict_table():
    ...
    try:
        ...
        if df_raw is None or df_raw.empty:
            return (
                jsonify({"status": "error", "message": f"表 {table_name} 无可用数据"}),
                400,
            )
            ...
            if allowed_in_df:
                return (
                    jsonify(
                        {
                            "status": "error",
                            "message": f'请指定目标列（可选: {", ".join(allowed_in_df)}）',
                        }
                    ),
                    400,
                )
        ...
        if target_column not in df_proc.columns:
            return (
                jsonify(
                    {
                        "status": "error",
                        "message": f"目标列 {target_column} 不存在于表 {table_name}",
                    }
                ),
                400,
            )
        ...
        if allowed_in_df and target_column not in allowed_in_df:
            return (
                jsonify(
                    {
                        "status": "error",
                        "message": f'目标列必须从 {", ".join(allowed_in_df)} 中选择',
                    }
                ),
                400,
            )
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            X_all.values,
            y_all.values,
            test_size=max(min(test_size, 0.9), 0.05),
            random_state=42,
        )
        ...
        try:
            ...
            for i in range(n):
                row = {
                    ...
                    "actual": (
                        float(y_all.iloc[i]) if not pd.isna(y_all.iloc[i]) else None
                    ),
                }
            ...
            try:
                ...
                if "student_id" in df_proc.columns:
                    ...
                    if st is not None and not st.empty and "student_id" in st.columns:
                        ...
                        dfj = df_join.merge(
                            st[["student_id", "grade"]], on="student_id", how="left"
                        )
                        if "grade" in dfj.columns:
                            ...
                            grp = (
                                dfj2.groupby("grade", dropna=False)
                                .agg(mae=("abs_err", "mean"), count=("abs_err", "size"))
                                .reset_index()
                            )
                            error_by_grade = [
                                {
            ...
            jsonify(
                {
                    ...
                    "data": {
                        ...
                        "model_results": {
                            k: {kk: to_py(vv) for kk, vv in vv.items()}
                            for k, vv in model_results.items()
                        },
                        "best_params": (
                            {k: to_py(v) for k, v in best_params.items()}
...
def predict_student():
    ...
    try:
        ...
        if not table_name or not student_id:
            return (
                jsonify(
                    {"status": "error", "message": "缺少参数: table 或 student_id"}
                ),
                400,
            )
        ...
        if "student_id" not in df_raw.columns:
            return (
                jsonify(
                    {
                        "status": "error",
                        "message": f"表 {table_name} 缺少 student_id 列，无法为学生预测",
                    }
                ),
                400,
            )
                ...
                if not nums:
                    return (
                        jsonify(
                            {
                                "status": "error",
                                "message": "无法识别目标列，请提供 targetColumn 或在表中包含成绩列（如“总成绩/总分/分数”等）",
                            }
                        ),
                        400,
                    )
        ...
        if target_column not in df_proc.columns:
            return (
                jsonify(
                    {"status": "error", "message": f"目标列 {target_column} 不存在"}
                ),
                400,
            )
        ...
        if not sort_done:
            if "exam_date" in row.columns:
                row["exam_date_parsed"] = pd.to_datetime(
                    row["exam_date"], errors="coerce"
                )
    ...
    try:
        ...
        if ug is not None and not ug.empty:
            try:
                ...
                df["calculus_avg_score"] = pd.to_numeric(
                    df.get("calculus_avg_score"), errors="coerce"
                )
                overall = {
                    ...
                    "total_courses": None,
                    "avg_score": (
                        float(df["calculus_avg_score"].dropna().mean())
                        if df["calculus_avg_score"].notna().any()
                        else None
                    ),
                    # 为前端提供明确的字段名，避免歧义
                    "calculus_avg_score": (
                    ...
                    "min_score": (
                        float(df["calculus_avg_score"].dropna().min())
                        if df["calculus_avg_score"].notna().any()
                        else None
                    ),
                    "max_score": (
                        float(df["calculus_avg_score"].dropna().max())
                        if df["calculus_avg_score"].notna().any()
                        else None
                    ),
                }
                ...
                return (
                    jsonify(
                        {
                            "status": "success",
                            "data": {"overall": overall, "by_semester": []},
                        }
                    ),
                    200,
                )
            ...
            if hg is not None and not hg.empty:
                try:
                    ...
                    df["total_score"] = pd.to_numeric(
                        df.get("total_score"), errors="coerce"
                    )
                    grp = (
                    ...
                    grp = grp.sort_values(
                        ["academic_year", "semester"], ascending=[False, True]
                    ).head(10)
                    semester_stats = [
                        {
            ...
            if hg is not None and not hg.empty:
                try:
                    ...
                    overall = {
                        ...
                        "total_courses": (
                            int(df["course_id"].nunique())
                            if "course_id" in df.columns
                            else None
                        ),
                        "avg_score": (
                            float(df["total_score"].dropna().mean())
                            if df["total_score"].notna().any()
                            else None
                        ),
                        "min_score": (
                            float(df["total_score"].dropna().min())
                            if df["total_score"].notna().any()
                            else None
                        ),
                        "max_score": (
                            float(df["total_score"].dropna().max())
                            if df["total_score"].notna().any()
                            else None
                        ),
                    }
        ...
        return (
            jsonify(
                {
                    "status": "success",
                    "data": {"overall": overall, "by_semester": semester_stats},
                }
            ),
            200,
        )
