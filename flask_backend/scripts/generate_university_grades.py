...
OUT_PATH_PUBLIC = os.path.join(
    ROOT_DIR, "vue_frontend", "public", "data", "university_grades.csv"
)
...
def synthesize_row(student: Dict[str, str]) -> Dict[str, Optional[str]]:
    ...
    math_score = clip(
        0.6 * uniform_target + 0.4 * (base_math + random.gauss(0, 14)), 20, 100
    )
...
def main(allow_missing: bool = False, count: int = 10000):
    ...
    print(
        f"[OK] Generated {len(rows)} university grades to {OUT_PATH} and {OUT_PATH_PUBLIC}"
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate synthetic university grades dataset"
    )
    parser.add_argument(
        "--allow-missing",
        action="store_true",
        help="If set, generate missing values according to probabilities",
    )
    parser.add_argument(
        "--count",
        type=int,
        default=10000,
        help="Number of grade records to generate (will not exceed students.csv rows)",
    )
